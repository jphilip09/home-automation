...
// Write the ssid and password to EEPROM
...
void setup() {
  ...
  print_elapsed();
  start_timing();
  eeprom_write_string(START_ADDR, SSID);
  print_elapsed();
  start_timing();
  eeprom_write_string(START_ADDR+strlen(SSID)+1, PWD);
  print_elapsed();
  EEPROM.commit();
  EEPROM.end();
  ...
  start_timing();
  eeprom_read_string(START_ADDR, buf, BUFSIZE);
  print_elapsed();
  Serial.print("String read: '");
  Serial.print(buf);
  Serial.println("'");
  start_timing();
  eeprom_read_string(START_ADDR+strlen(SSID)+1, buf, BUFSIZE);
  print_elapsed();
  Serial.print("String read: '");
  Serial.print(buf);
  Serial.println("'");
